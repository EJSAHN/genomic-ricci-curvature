...
def best_single_and_pair_reconstruction(
    ...
    """Find the best one- and two-source reconstructions without Python pair loops.

    This is algebraically identical to the earlier implementation: each pair is
    fitted with a least-squares convex weight constrained to [0, 1], and errors
    are calculated only over markers callable in the target and relevant source
    centroid(s). Vectorization is important for the 28-source external panel.
    """
    y = np.asarray(target, dtype=float)
    controls = np.asarray(control_matrix, dtype=float)
    n_controls = int(len(control_names))
    if controls.ndim != 2 or controls.shape[0] != n_controls:
        raise ValueError("Control-centroid matrix/name mismatch")

    target_finite = np.isfinite(y)

    # Single-source reconstruction.
    single_mask = np.isfinite(controls) & target_finite[None, :]
    single_overlap = single_mask.sum(axis=1).astype(int)
    single_diff = np.where(single_mask, controls - y[None, :], 0.0)
    single_sse = np.square(single_diff).sum(axis=1)
    single_error = np.full(n_controls, np.inf, dtype=float)
    valid_single = single_overlap >= int(min_overlap)
    single_error[valid_single] = np.sqrt(
        single_sse[valid_single] / single_overlap[valid_single]
    )
    if np.isfinite(single_error).any():
        single_idx = int(np.argmin(single_error))
        best_single_error = float(single_error[single_idx])
        best_single_name = str(control_names[single_idx])
        best_single_overlap = int(single_overlap[single_idx])
    else:
        best_single_error = float("nan")
        best_single_name = ""
        best_single_overlap = 0

    # Two-source convex reconstruction.
    pair_a, pair_b = np.triu_indices(n_controls, k=1)
    if len(pair_a) == 0:
        ...
        a = controls[pair_a]
        b = controls[pair_b]
        pair_mask = np.isfinite(a) & np.isfinite(b) & target_finite[None, :]
        pair_overlap = pair_mask.sum(axis=1).astype(int)
        direction = np.where(pair_mask, a - b, 0.0)
        y_minus_b = np.where(pair_mask, y[None, :] - b, 0.0)
        denom = np.square(direction).sum(axis=1)
        numer = (y_minus_b * direction).sum(axis=1)
        weights = np.full(len(pair_a), 0.5, dtype=float)
        nonzero = denom > 1e-15
        weights[nonzero] = np.clip(numer[nonzero] / denom[nonzero], 0.0, 1.0)
        residual = y_minus_b - weights[:, None] * direction
        pair_sse = np.square(residual).sum(axis=1)
        pair_error = np.full(len(pair_a), np.inf, dtype=float)
        valid_pair = pair_overlap >= int(min_overlap)
        pair_error[valid_pair] = np.sqrt(
            pair_sse[valid_pair] / pair_overlap[valid_pair]
        )
        if np.isfinite(pair_error).any():
            pair_idx = int(np.argmin(pair_error))
            best_pair_error = float(pair_error[pair_idx])
            best_pair_a = str(control_names[int(pair_a[pair_idx])])
            best_pair_b = str(control_names[int(pair_b[pair_idx])])
            best_weight = float(weights[pair_idx])
            best_pair_overlap = int(pair_overlap[pair_idx])
        else:
            best_pair_error = float("nan")
            best_pair_a = ""
            best_pair_b = ""
