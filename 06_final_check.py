...
'mixture_score' against the known ground truth labels (pools passed via --true_pools).
...
# Pass the comma-separated SRR IDs known to be pools via --true_pools.
DEFAULT_TRUE_POOLS = "SRR17037621,SRR17037622,SRR17037623"

def run_validation(results_dir, true_pools):
    true_pools = set([str(s).strip().upper() for s in true_pools if str(s).strip()])
    ...
    # Mark Ground Truth
    df['is_true_pool'] = df['sample'].apply(lambda x: 'Pool (Ground Truth)' if str(x).strip().upper() in true_pools else 'Individual')
...
def main():
    ...
    ap.add_argument("--true_pools", default=DEFAULT_TRUE_POOLS,
                    help="Comma-separated SRR IDs treated as pools (ground truth).")
    args = ap.parse_args()

    run_validation(args.results_dir, args.true_pools.split(','))
